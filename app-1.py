...
# 3. 初始化狀態 (解決 NameError 的關鍵)
if 'last_res' not in st.session_state:
    st.session_state.last_res = None

# 4. 側邊欄：Model Information
with st.sidebar:
...
with tab1:
    ...
    with col_res:
        if st.session_state.last_res:
            ...
            with st.expander("📝 查看語意正規化原文"):
                st.info(res['trans'])
        else:
            st.info("💡 請貼入內容並啟動掃描，系統將展示 XAI 判斷依據。")